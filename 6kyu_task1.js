/* 
Gantulga Enkhbat - CodeWars "6Kyu solutions" 
...
1. You live in the city of Cartesia where all roads are laid out in a perfect grid. 
...
function isValidWalk(walk) {
    let nLen = walk.filter(item=>item==='n').length
    let sLen = walk.filter(item=>item==='s').length
    let wLen = walk.filter(item=>item==='w').length
    let eLen = walk.filter(item=>item==='e').length
    
    return walk.length === 10 && nLen === sLen && wLen === eLen
  }